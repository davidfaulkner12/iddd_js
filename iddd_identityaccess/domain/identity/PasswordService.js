...
class PasswordService extends AssertionConcern {
    ...
    generateStrongPassword() {
        ...
        while (!isStrong) {
            ...
            case 1:
                index = getRandomInt(LETTERS.length);
                password = password + LETTERS.substring(index, index+1).toLowerCase()
                break;
            case 2:
                index = getRandomInt(DIGITS.length);
                password = password + DIGITS.substring(index, index+1)
                break;
            case 3:
                index = getRandomInt(SYMBOLS.length);
                password = password + SYMBOLS.substring(index, index+1)
                break;
            }
    ...
    isStrong(aPlainTextPassword) {
        return this._calculatePasswordStrength(aPlainTextPassword) >= STRONG_THRESHOLD;
    }

    isVeryStrong(aPlainTextPassword) {
        return this._calculatePasswordStrength(aPlainTextPassword) >= VERY_STRONG_THRESHOLD;
    }
