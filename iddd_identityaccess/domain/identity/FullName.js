...
class FullName extends AssertionConcern {
  constructor() {
    ...
    } else if (arguments.length == 2 && _.isString(arguments[0]) && _.isString(arguments[1])) {
      this.firstName = arguments[0]
      this.lastName = arguments[1]
    } else {
      throw new Error("IllegalArgument: FullName must be called with either a FullName to copy or two Strings")
    }
  }
